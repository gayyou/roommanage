...
export default class ReservationOrder extends Vue {
  get reservationOrderList() {
    return orderManage.reservationList;
  }
  ...
  changeFilteredData() {
    this.filteredData = [];
    let count = 1;
    for (let i = 0; i < this.reservationOrderList.length; i++) {
      let item = this.reservationOrderList[i];
  ...
  beforeMount() {
    ...
    let tableWidthList: number[] = [150, 220, 250, 200, 195, 200, 200, 180];  // 总的1600
